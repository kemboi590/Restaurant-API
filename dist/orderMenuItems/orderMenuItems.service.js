...
const getOrderMenuItemsService = async () => {
    const orderMenuItems = await db_1.db.query.orderMenuItemTable.findMany();
...
const getOrderMenuItemByIdService = async (id) => {
    const orderMenuItem = await db_1.db.query.orderMenuItemTable.findFirst({
...
const createOrderMenuItemService = async (orderMenuItem) => {
    await db_1.db.insert(schema_1.orderMenuItemTable).values(orderMenuItem);
...
const updateOrderMenuItemService = async (id, orderMenuItem) => {
    await db_1.db.update(schema_1.orderMenuItemTable).set(orderMenuItem).where((0, drizzle_orm_1.eq)(schema_1.orderMenuItemTable.id, id));
...
const deleteOrderMenuItemService = async (id) => {
    await db_1.db.delete(schema_1.orderMenuItemTable).where((0, drizzle_orm_1.eq)(schema_1.orderMenuItemTable.id, id));
